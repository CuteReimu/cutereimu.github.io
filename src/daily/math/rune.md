...
- 当连续未出至少 4 个的次数达到 9 时，本次强制出 4 个；其中以 $2/3$ 概率仍为 4 个，以 $1/3$ 概率升为 5 个。
- 当连续未出 5 个的次数达到 19 时，本次强制出 5 个。

目标是计算完成三组同色同字所需洗练次数的数学期望。
...
$$
X\sim \mathrm{Binomial}\left(n,\frac{1}{45}\right)
$$
...
P_2^{(n)}
=
1-\left(\frac{44}{45}\right)^n
-
n\cdot\frac{1}{45}\left(\frac{44}{45}\right)^{n-1}
$$
...
$$
Y\sim \mathrm{Binomial}\left(n,\frac89\right)
$$
...
$$
\mathbb E[T]\approx 1.42\times 10^3
$$
