...
注意我们代码中的高亮行，如果某个猜测的反馈结果中有3个○，也就是完全正确，我会将它的信息量增加一个固定的值。为什么要这样做呢？考虑这样一种情况，假设我们的某个猜测不是正确答案，但是在这个猜测之后已经确定了正确答案，接下来我们仍然要浪费1次次数去猜测正确答案，所以理应给直接猜中正确答案增加一些信息量加以奖励。增加的数字`10`是测试之后得到的一个经验值，最终测试次数的数学期望最小。你可以根据实际情况调整这个经验值。
...
我们来看一个类似的问题：给你9颗小石头和一架没有砝码的天平，其中有8颗石头重量一样，另外一个比这8颗略重。请问在最坏情况下，最少要称重几次，才能把这颗较重的石头找出来？

回顾一下**信息熵**的性质：当所有可能事件的概率相等时，熵最大。

!!我们只需要将9颗石头分成3组，每组3颗，选其中的两组称重一次，有三种情况，左边重，右边重，或者两边一样重，概率都是1/3，**信息熵**最大，这样一次就可以求出较重的石头在哪三颗中。接下来我们如法炮制，取这三颗石头中的任意两颗称重一次，即可得到答案。!!