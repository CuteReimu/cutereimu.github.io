...
本路线是Any%速通玩家们最早使用的速通路线，直到现在仍然很流行，但并不是目前最快的路线。目前最快的路线是[Any%速通路线（酸泪线）](any-isma.md)。相比于酸泪线而言，本路线的难度会低很多。

Any%速通不可避免需要在只拥有0钉白波的情况下进行守望者骑士、乌姆、空洞骑士的战斗，难度较大。如果新手玩家难以达成，但又非常想参与Any%速通的话，可以考虑使用[Any%速通路线（黑波线）](any-shade-soul.md)，在本路线的基础上变化不大，方便在熟练之后过渡到本路线。

:::

Any% 1.4.3.2+版本速通路线（超冲线），包含两个分支，仅在最后一部分路线上略有区别。我们先介绍较快的[王后花园酸冲线](#王后花园酸冲线)，在文末会补充介绍[替代路线：深巢温泉/蘑菇孢子线](#替代路线-深巢温泉-蘑菇孢子线)。
...
- 击败格鲁兹之母（建议保留两白波的灵魂用于清场）（+50吉欧）
- 在萨鲁巴的商店门口的长椅休息，购买萨满之石（-220吉欧）
...
- 酸冲进入王后花园（需要大量练习）
- 螳螂佩特拉遭遇战（佩特拉共45血，需2白波+1骨钉，若白波失误需调整策略，容易翻车）
- 向左下进入深邃巢穴入口
...
## 替代路线：深巢温泉/蘑菇孢子线
在1.4.3.2版本中，蘑菇孢子会对空洞骑士造成大量伤害，在1.5版本中修复了这个BUG。并且，由于王后花园酸冲和螳螂佩特拉遭遇战的难度都非常高，容易在此处损失时间，因此深巢温泉/蘑菇孢子线也是降低难度的一种可行路线。不考虑捡取蘑菇孢子的话这条路线会慢40~45秒，在1.4.3.2版本中捡取蘑菇孢子的话则只会慢20秒左右（以上的时间是估算的结果，假设王后花园酸冲线发挥得很好的情况下）。
...
- 离开教师档案馆后左行
- 向下走，右转通过王后驿站进入真菌荒地
- 从第一个岔路口下落，再下落经过一个过渡房间，左行至蘑菇孢子房间
- <Badge text="仅1.4.3.2版本" type="tip" vertical="baseline" /> 拾取蘑菇孢子并跳起来撞荆棘
- 从左边出去，坠入深邃巢穴

### 深邃巢穴移动
- 落地后（可以用冲刺取消坠落硬直）右行至底部
- 下穿加皮德房间，进入深巢温泉（如果需要，可以坐一下长椅）
- 向左进入下一个房间，然后进入左上方的入口
- 一直向左上方走，穿过两个房间
- 最后一个房间和王后花园酸冲线相同，从左侧出去进入遥远的村庄

### 前往黑卵
- <Badge text="仅1.4.3.2版本" type="tip" vertical="baseline" /> 离开野兽巢穴前，坐椅子，卸下萨满之石换蘑菇孢子（不要过载，你的灵魂将用于治疗）

### 空洞骑士
- <Badge text="仅1.4.3.2版本" type="tip" vertical="baseline" /> 拥有蘑菇孢子的情况下，战斗技巧将会有很大的区别
  - 该护符产生孢子云时，它会进入一个冷却，但如果你受到伤害，冷却将会重置。所以最理想的情况是，你可以回血，受伤，然后再回血，以造成更多的伤害
