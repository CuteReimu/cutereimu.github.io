...
你可以前往[**计时器生成器**的Gitee仓库](https://gitee.com/CuteReimu/hksplitmaker/releases)下载，你也可以下载仓库代码后自行编译。
...
2. 你可以选择一个已有的模板或者打开一个`*.lss`文件，也可以自己从头开始编辑
   ...
   - 如果你是速通老手，有自己对路线的理解，想要自己编辑模板，可以参考以下说明
      ...
      3. 如果是一个全关速通或者万神殿某一门的速通，不需要勾选第一行的`自定义开始节点`
      4. 如果最后一个节点不是游戏结束，则需要勾选最后一行的`不是游戏结束节点`，**否则不要勾选**
