...
现在，允许使用丝之歌自带的回档功能跳过开场动画。操作方法是：在存档选择界面，在任意一个现有存档下方点击回档按钮，下滑到最下方，选择`第一幕开启`。

同时你需要对计时器做出以下改动：
- 将开始计时的触发条件从 `Start New Game (Start)` 改为 `Act 1 Start (Start)` 。
  - 译者注：你可以使用[计时器生成器](sssplitmaker-faq.md)打开你原来的板子（.lss文件），把开始计时的触发条件从 `开始新游戏（开始）` 改为 `第一幕开始（开始）`，另存为后，重新用 **LiveSplit** 打开即可。
