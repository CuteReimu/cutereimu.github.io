...
2. 进入 steam 控制台，在下方输入
    ...
    在`download_depot`后一共有3个数字，`1030300`表示丝之歌，`1030301`表示 Windows，最后面一长串数字是版本对应的序号。这3个数字都不要漏了。
    
    :::

4. 回车，等待下载
5. 下载完成后，在 steam 控制台上找到输出的文件路径，在电脑中打开这个路径
...
7. 在 steam 中找到 `添加非steam游戏` ，在弹出的窗口中点击 `浏览` 按钮，选中 `depot_1030301` 文件夹中带有丝之歌图标的 `Hollow Knight Silksong.exe`
