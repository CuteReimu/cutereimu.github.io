...
打开**LiveSplit** &rarr; 右键 &rarr; Edit Layout，你会看到有很多个组件。找到其中的 Splits ，将其删掉，然后点击左边的加号，新增一个 SubSplits 组件，放在相同位置。这样设置后，你的**LiveSplit**就支持折叠分段了。

经过上述设置后，编辑你的模板（*.lss）文件，在你想要折叠进去的分段名前面加上`-`，这个分段就变成了小分段，小分段会折叠入其下方的一个大分段。

其实计时器生成器的很多长流程模板中都已经帮你加好了`-`。
...
从小分段上看，依旧是A、B、C、D、E五个分段。由于C的分段名前面没有加`-`，因此会将A、B、C折叠为一个大分段，大分段名默认是C。同理，D、E折叠为一个大分段，大分段名是E。

换句话说，**大分段名默认使用最后这个不加`-`的小分段名**。

那么如何只修改大分段名，保持小分段名不变呢？只需把`C`改为`{XX}C`，这样小分段名还是C，而折叠外面的大分段名就变成`XX`了。
...
在防火墙中设置禁止 `LiveSplit.exe` 联网即可。此方法不一定 100% 有效，但值得一试。

原理是，**LiveSplit**每次启动时都会检测更新（包括**LiveSplit**本身和各个游戏计时插件的版本更新），更新成功或失败后才会打开**LiveSplit**界面。更新地址本身访问较慢，因此会卡很久，禁止联网可以让它立即就失败，界面就打开了。当然，你也可以尝试使用其它方法达到类似的效果（例如修改`/etc/hosts`让访问更新地址立即失败）。
