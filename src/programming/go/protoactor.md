...
func (ctx *actorContext) handleStop() {
    ...
    ctx.InvokeUserMessage(stoppingMessage) // 这里直接调用 InvokeUserMessage，交给用户实现的 Receive 方法去处理 Stopping 消息
    ctx.stopAllChildren()
...
func (ctx *actorContext) finalizeStop() {
    ...
    ctx.InvokeUserMessage(stoppedMessage) // 这里直接调用 InvokeUserMessage，交给用户实现的 Receive 方法去处理 Stopped 消息
