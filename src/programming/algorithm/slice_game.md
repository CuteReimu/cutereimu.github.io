...
首先我们要解决的问题是，需要判断它有没有解。这是一个纯数学问题，有固定的方法：我们将3×3网格按从左到右、从上到下展开成一维序列，忽略空格，然后计算其中**逆序对**的数量，**逆序对**为偶数就有解，奇数就无解。

举两个例子：
- 假设展开后得到的一维序列是12365478，其中6和5、6和4、5和4的顺序反了，也就是有3个**逆序对**，为奇数，因此这个状态是没有解的。
- 文章开头的那个例子展开是34528167，一共有10个**逆序对**，为偶数，因此这个状态是有解的。

对于一个有解的数字华容道，我们就可以用**广度优先遍历**的方法来解决这个问题了。我们从初始状态开始，只有可能空格相邻的格子移动到空格上，于是我们可以用一个队列来存储这些移动方法。接下来，我们从队列中取出一个移动方法，从这个新的状态开始继续考虑，同样只有可能空格相邻的格子移动到空格上，我们继续将这些可能的操作加入到队列中去，重复这个过程，直到我们找到一个状态是初始状态1-8排好为止。当然了，在这之中，我们需要记下已经遍历过的状态，以免重复遍历。前面所说的队列就是广度优先遍历的**开集**，后面记录已经遍历过的状态就是**闭集**。如果对这个算法还不熟悉，可以参考[《Dijkstra寻路算法》](dijkstra.md)这篇文章。
