...
那么问题就来了，如何让请求协议和返回协议一一对应呢？其中一个方案就是提前约定好，例如采用“奇偶法”，请求协议的`type`用`2N`，对应的返回协议的`type`就使用`2N+1`。或者进一步地，由于**请求协议**一定是客户端&rarr;服务器，而**返回协议**一定是服务器&rarr;客户端，它们之间不会发生冲突，完全可以将一一对应的协议使用相同的`type`值。这是目前非常常用的一种解决方案。

我们继续考虑另外一个设计问题：如果服务端和客户端约定，当客户端进行一个非法请求后（例如玩家钱不够仍然要买东西），服务端返回一个**通用的错误协议**，这样客户端可以非常容易地根据**通用的错误协议**弹窗提示用户。这是一个十分合理的设计，可以降低客户端的设计复杂度，但会带来一个新的问题：如果客户端连续发多条请求，服务端返回一个**通用的错误协议**，客户端怎么知道它对应哪一个请求呢？

为了解决这个问题，也有一个非常通用的方案，客户端在**请求协议**中加一个编号，我们以JSON为例：
...
服务端收到请求后，发送返回协议时，将编号原封不动返回回去，客户端就可以精准对应了。

对于protobuf，我们也可以在前面合适的位置添加2个字节，作为`seq`，例如：

```mermaid
<!-- @include: proto_3.mermaid -->
```
